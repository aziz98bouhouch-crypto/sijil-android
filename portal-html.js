...
function applyScan(raw){raw=String(raw||"").trim();if(!raw){toast("لم يُعثر على رمز صالح");return}var host="";try{var u=new URL(raw);host=u.protocol+"//"+u.host;}catch(e){host=raw.replace(/\\/+$/,"")}CFG.lan=host.replace(/\\/+$/,"");if(!CFG.mode||CFG.mode==="auto")CFG.mode="auto";saveCfg();$x("modes").style.display="flex";toast("تم الربط: "+CFG.lan);if($x("settings").style.display==="block")S.closeSettings();if($x("code").value.trim())S.login()}
S.scan=function(){
